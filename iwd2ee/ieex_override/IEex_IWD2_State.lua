		...
		if resWrapper:isValid() then
			...
			for header = 1, numHeaders, 1 do
				...
				if IEex_ReadByte(offset, 0x0) == 1 and IEex_GetDistance(sourceX, sourceY, targetX, targetY) <= itemRange then
					...
					local isHit = false
					local stateValue = bit.bor(IEex_ReadDword(creatureData + 0x5BC), IEex_ReadDword(creatureData + 0x920))
					if attackRoll >= 20 or bit.band(stateValue, 0xE9) > 0 then
						isHit = true
					elseif attackRoll >= 2 then
						...
						if bit.band(IEex_ReadDword(offset + 0x26), 0x1) > 0 then
...
--]]
		if the_timing ~= 1 and the_timing ~= 2 and the_timing ~= 9 and the_opcode ~= 124 and the_opcode ~= 500 and ((the_duration - time_applied < 15 and bit.band(the_internal_flags, 0x200000) == 0) or the_opcode == 119 or the_opcode == 218) then
			IEex_WriteDword(eData + 0xCC, bit.bor(the_internal_flags, 0x200000))
...
--]]
	...
	if seconds_ago > 0 then
		...
		IEex_IterateActorEffects(targetID, function(eData)
			...
			if the_time_applied < time_applied and the_timing ~= 1 and the_timing ~= 2 and the_timing ~= 9 and the_parent_resource ~= parent_resource and the_opcode ~= 500 and bit.band(the_internal_flags, 0x400000) == 0 then
				local the_duration = IEex_ReadDword(eData + 0x28)
...
function MEDEFLEC(originatingEffectData, effectData, creatureData)
	...
	if opcode == 12 and effectRES == "" and (damage_type == 0 and bit.band(types_blocked, 0x4000) > 0) or (damage_type ~= 0 and bit.band(types_blocked, damage_type) > 0) and ((bit.band(savingthrow, 0x10000) > 0 and delay ~= 0) or (bit.band(savingthrow, 0x10000) == 0 and doDeflect)) then
